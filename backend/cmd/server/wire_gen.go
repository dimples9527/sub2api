...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	customPlatformRepository := repository.NewCustomPlatformRepository(db)
	customPlatformService := service.NewCustomPlatformService(customPlatformRepository)
	customPlatformHandler := admin.NewCustomPlatformHandler(customPlatformService)
	...
	monitorGroupPlatformOverrideRepository := repository.NewMonitorGroupPlatformOverrideRepository(db)
	monitorGroupPlatformOverrideService := service.NewMonitorGroupPlatformOverrideServiceWithCustomPlatform(monitorGroupPlatformOverrideRepository, customPlatformService)
	...
	supplierProviderTokenRedisCache := repository.NewSupplierProviderTokenCache(redisClient)
	supplierCostDeviationSettingRepository := repository.NewSupplierCostDeviationSettingRepository(db)
	supplierCostDeviationSettingsService := service.NewSupplierCostDeviationSettingsService(supplierCostDeviationSettingRepository)
	supplierCostSourceRepository := repository.NewSupplierCostSourceConfigRepository(db)
	supplierCostSourceConfigService := service.ProvideSupplierCostSourceConfigService(supplierCostSourceRepository)
	supplierProviderService := service.ProvideSupplierProviderService(supplierProviderRepository, secretEncryptor, supplierProviderTypeRepository, supplierProviderTokenRedisCache, supplierCostDeviationSettingsService, supplierCostSourceConfigService)
	supplierProviderHandler := admin.NewSupplierProviderHandler(supplierProviderService)
	supplierProviderAuthAuditRepository := repository.NewSupplierProviderAuthAuditRepository(db)
	supplierProviderAuthAuditService := service.NewSupplierProviderAuthAuditService(supplierProviderAuthAuditRepository, supplierProviderTokenRedisCache)
	supplierProviderAuthHandler := admin.NewSupplierProviderAuthHandler(supplierProviderAuthAuditService)
	...
	supplierProviderRechargeRepository := repository.NewSupplierProviderRechargeRepository(db)
	supplierProviderRemoteRegistry := service.ProvideSupplierProviderRemoteClient(supplierProviderTokenRedisCache, settingService, supplierProviderAuthAuditService)
	...
	supplierProviderCostReviewRepository := repository.NewSupplierProviderCostReviewRepository(db)
	supplierProviderCostReviewService := service.ProvideSupplierProviderCostReviewService(supplierProviderCostReviewRepository)
	supplierCostAlertRepository := repository.NewSupplierCostAlertRepository(db)
	...
	supplierCostAlertService := service.ProvideSupplierCostAlertService(supplierCostAlertRepository, supplierNotificationDispatcher)
	supplierProviderSyncService := service.ProvideSupplierProviderSyncService(supplierProviderRepository, supplierProviderDataRepository, supplierProviderRechargeRepository, supplierProviderRemoteRegistry, secretEncryptor, supplierProviderTokenRedisCache, supplierProviderGroupMatcher, supplierCostDeviationSettingsService, supplierCostSourceConfigService, supplierProviderCostReviewService, supplierCostAlertService, supplierNotificationDispatcher)
	supplierProviderSyncHandler := handler.ProvideSupplierProviderSyncHandler(supplierProviderSyncService, supplierProviderDataRepository, supplierProviderGroupMatcher, supplierGroupGuardReconciler, customPlatformService, monitorGroupPlatformOverrideService, sessionLimitCache, accountRepository)
	supplierProviderRemoteRechargeHistoryClient := service.ProvideSupplierProviderRechargeRemoteClient(supplierProviderRemoteRegistry)
	supplierProviderRechargeService := service.ProvideSupplierProviderRechargeService(supplierProviderRepository, supplierProviderRechargeRepository, supplierProviderRemoteRechargeHistoryClient, secretEncryptor)
	supplierProviderRechargeHandler := handler.ProvideSupplierProviderRechargeHandler(supplierProviderRechargeService)
	...
	supplierAccountRateGuardRateSyncer := service.ProvideSupplierAccountRateGuardRateSyncer(supplierProviderSyncService)
	supplierAccountRateGuardRepository := repository.NewSupplierAccountRateGuardRepository(db)
	accountRateGuardGroupRemover := repository.NewAccountRateGuardRepository(db)
	supplierAccountRateGuardService := service.NewSupplierAccountRateGuardService(supplierProviderRepository, supplierAccountRateGuardRateSyncer, supplierAccountRateGuardRepository, accountRateGuardGroupRemover)
	supplierAccountHealthGuardRepository := repository.NewSupplierAccountHealthGuardRepository(db)
	supplierAccountHealthGuardAccountStore := service.ProvideSupplierAccountHealthGuardAccountStore(accountRepository)
	supplierAccountHealthGuardTester := service.ProvideSupplierAccountHealthGuardTester(accountTestService)
	supplierAccountHealthHistoryRepository := repository.NewSupplierAccountHealthHistoryRepository(db)
	supplierAccountHealthGuardService := service.ProvideSupplierAccountHealthGuardService(supplierAccountHealthGuardRepository, supplierAccountHealthGuardAccountStore, supplierAccountHealthGuardTester, supplierAccountHealthHistoryRepository)
	supplierGroupSchedulingElectionRepository := repository.NewSupplierGroupSchedulingElectionRepository(db)
	supplierGroupSchedulingElectionAccountStore := service.ProvideSupplierGroupSchedulingElectionAccountStore(accountRepository)
	supplierGroupSchedulingElectionService := service.ProvideSupplierGroupSchedulingElectionService(supplierGroupSchedulingElectionRepository, supplierGroupSchedulingElectionAccountStore)
	supplierAutomationService := service.ProvideSupplierAutomationService(supplierAutomationRepository, supplierAutomationRedisLock, supplierProviderSyncService, supplierProviderDataRepository, supplierProviderRechargeService, supplierRateGuardService, supplierAccountRateGuardService, supplierAccountHealthGuardService, supplierAccountRateGuardRepository, supplierGroupSchedulingElectionService)
	...
	supplierCostAlertHandler := admin.NewSupplierCostAlertHandler(supplierCostAlertService)
	supplierCostDeviationSettingsHandler := admin.NewSupplierCostDeviationSettingsHandler(supplierCostDeviationSettingsService)
	supplierCostSourceConfigHandler := admin.NewSupplierCostSourceConfigHandler(supplierCostSourceConfigService)
	supplierProviderCostReviewHandler := handler.ProvideSupplierProviderCostReviewHandler(supplierProviderCostReviewService)
	supplierAccountHealthTrendService := service.ProvideSupplierAccountHealthTrendService(supplierAccountHealthHistoryRepository)
	supplierAccountHealthHandler := handler.ProvideSupplierAccountHealthHandler(supplierAccountHealthTrendService)
	...
	upstreamBalanceStore := repository.NewUpstreamBalanceRepository(client, db)
	upstreamBalanceConsumptionService := service.ProvideUpstreamBalanceConsumptionService(upstreamBalanceStore, upstreamProviderService, settingRepository, usageLogRepository)
	upstreamAccountHealthGuardRecordStore := repository.NewUpstreamAccountHealthGuardRecordRepository(db)
	...
	modelMonitorGroupHealthRepository := repository.NewModelMonitorGroupHealthRepository(db)
	modelMonitorGroupHealthService := service.NewModelMonitorGroupHealthService(modelMonitorGroupHealthRepository)
	modelMonitorGroupHealthHandler := admin.NewModelMonitorGroupHealthHandler(modelMonitorGroupHealthService)
	openCodeGoUsageService := service.ProvideOpenCodeGoUsageService(accountRepository, httpUpstream, settingService, leaderLockCache, db)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, customPlatformHandler, accountHandler, customPlatformService, monitorGroupPlatformOverrideService, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, cnProviderHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, pluginHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, promptAdminHandler, paymentHandler, affiliateHandler, complianceHandler, auditLogHandler, upstreamBillingProbeService, supplierProviderHandler, supplierProviderAuthHandler, supplierProviderTypeHandler, supplierProviderSyncHandler, supplierProviderRechargeHandler, supplierAutomationHandler, supplierDashboardHandler, supplierBalanceAlertHandler, supplierNotificationHandler, supplierCostAlertHandler, supplierCostDeviationSettingsHandler, supplierCostSourceConfigHandler, supplierProviderCostReviewHandler, supplierAccountHealthHandler, upstreamProviderHandler, upstreamDashboardHandler, upstreamManagementHandler, upstreamAccountSyncHandler, modelMonitorGroupHealthHandler, ollamaCloudUsageService, openCodeGoUsageService)
	...
	modelSquareHandler := handler.NewModelSquareHandler(upstreamManagementService, channelService, adminService, monitorGroupPlatformOverrideService, billingService)
	...
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, modelSquareHandler, asyncImageHandler, batchImageHandler, idempotencyCoordinator, idempotencyCleanupService, openAIQuotaAutoResetService)
	...
	llmMonitorHistoryRepository := repository.NewLLMMonitorHistoryRepository(db)
	llmMonitorHistoryService := service.NewLLMMonitorHistoryService(llmMonitorHistoryRepository)
	engine := server.ProvideRouter(configConfig, handlers, jwtAuthMiddleware, optionalJWTAuthMiddleware, adminAuthMiddleware, apiKeyAuthMiddleware, auditLogMiddleware, stepUpAuthMiddleware, apiKeyService, subscriptionService, opsService, settingService, llmMonitorHistoryService, compositeRouteResolver, redisClient)
	...
	opsCleanupService := service.ProvideOpsCleanupService(opsRepository, db, redisClient, configConfig, channelMonitorService, llmMonitorHistoryService, settingRepository, opsService)
