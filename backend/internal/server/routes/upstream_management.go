...
func registerUpstreamManagementRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	upstream := admin.Group("/upstream-management")
	{
		...
		upstream.GET("/model-square/sync-accounts", h.Admin.Account.ListModelSquareSyncAccounts)
		...
		accounts := upstream.Group("/accounts")
		{
			...
			accounts.GET("/balance-consumption/recharges", h.Admin.UpstreamAccountSync.ListBalanceRecharges)
		...
		providers.GET("/balance-consumption/recharges", h.Admin.UpstreamAccountSync.ListBalanceRecharges)
