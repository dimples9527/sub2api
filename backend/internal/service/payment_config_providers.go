...
func (s *PaymentConfigService) ListProviderInstancesWithConfig(ctx context.Context) ([]ProviderInstanceResponse, error) {
	...
	for _, inst := range instances {
		...
		resp.Config, err = s.decryptAndMaskProviderConfig(inst.ProviderKey, inst.Config)
		if err != nil {
...
func (s *PaymentConfigService) GetProviderInstanceWithConfig(ctx context.Context, id int64) (ProviderInstanceResponse, error) {
	...
	cfg, err := s.decryptAndMaskProviderConfig(inst.ProviderKey, inst.Config)
	if err != nil {
...
// decryptAndMaskConfig returns the stored config with sensitive fields masked.
func (s *PaymentConfigService) decryptAndMaskConfig(encrypted string) (map[string]string, error) {
	...
	for k, v := range cfg {
		if isSensitiveProviderConfigField("", k) {
...
// decryptAndMaskProviderConfig returns the stored config with sensitive fields masked.
...
func (s *PaymentConfigService) decryptAndMaskProviderConfig(providerKey, encrypted string) (map[string]string, error) {
	cfg, err := s.decryptConfig(encrypted)
	if err != nil {
		return nil, err
	}
...
func isSensitiveProviderConfigField(providerKey, fieldName string) bool {
	...
	if providerKey != "" {
		...
		_, found := fields[normalized]
		return found
	}
	for _, fields := range providerSensitiveConfigFields {
		if _, found := fields[normalized]; found {
			return true
		}
	}
	return false
...
func (s *PaymentConfigService) hasSensitiveConfigChanges(ctx context.Context, id int64, newConfig map[string]string) (bool, error) {
	inst, err := s.entClient.PaymentProviderInstance.Get(ctx, id)
	if err != nil {
		return false, fmt.Errorf("load existing provider: %w", err)
	}
	existing, err := s.decryptConfig(inst.Config)
	if err != nil {
		return false, fmt.Errorf("decrypt existing config for instance %d: %w", id, err)
	}
	for key, value := range newConfig {
		if !isSensitiveProviderConfigField(inst.ProviderKey, key) {
			continue
		}
		if value == "" || value == maskedSensitiveConfigValue {
			continue
		}
		if providerConfigFieldValue(existing, key) != value {
			return true, nil
		}
	}
	return false, nil
...
func (s *PaymentConfigService) mergeConfig(ctx context.Context, id int64, newConfig map[string]string) (map[string]string, error) {
	...
	for k, v := range newConfig {
		...
		if (v == "" || v == maskedSensitiveConfigValue) && isSensitiveProviderConfigField(inst.ProviderKey, k) {
			continue
		}
		existing[k] = v
	}
