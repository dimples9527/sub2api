...
// MonitorGroupPlatformOverride 存储仅供模型监控使用的分组展示配置。
type MonitorGroupPlatformOverride struct {
	ActualPlatform string
	ShowInMonitor  bool
}

// MonitorGroupPlatformOverrideRepository 存储仅供模型监控使用的分组展示配置。
type MonitorGroupPlatformOverrideRepository interface {
	ListByGroupIDs(ctx context.Context, groupIDs []int64) (map[int64]MonitorGroupPlatformOverride, error)
	Set(ctx context.Context, groupID int64, platform string) error
	SetShowInMonitor(ctx context.Context, groupID int64, show bool) error
	Clear(ctx context.Context, groupID int64) error
}

// MonitorGroupPlatformOverrideService 管理模型监控专用的分组展示配置。
...
func (s *monitorGroupPlatformOverrideService) ListByGroupIDs(ctx context.Context, groupIDs []int64) (map[int64]MonitorGroupPlatformOverride, error) {
	if s == nil || s.repo == nil {
		return map[int64]MonitorGroupPlatformOverride{}, nil
	}
...
func (s *monitorGroupPlatformOverrideService) SetShowInMonitor(ctx context.Context, groupID int64, show bool) error {
	if groupID <= 0 {
		return fmt.Errorf("group id must be positive")
	}
	if s == nil || s.repo == nil {
	...
	return s.repo.SetShowInMonitor(ctx, groupID, show)
}
