...
func (s *SettingService) GetPublicSettings(ctx context.Context) (*PublicSettings, error) {
	...
	return &PublicSettings{
		...
		LLMMonitorProviderURL:            s.llmMonitorProviderURL(settings),
...
func (s *SettingService) GetPublicSettingsForInjection(ctx context.Context) (any, error) {
	...
	return &struct {
		...
		LLMMonitorProviderURL            string          `json:"llm_monitor_provider_url,omitempty"`
...
func (s *SettingService) UpdateSettings(ctx context.Context, settings *SystemSettings) error {
	...
	updates[SettingKeyLLMMonitorProviderURL] = strings.TrimSpace(settings.LLMMonitorProviderURL)
...
func (s *SettingService) llmMonitorStatusAPIURL(settings map[string]string) string {
	if value := strings.TrimSpace(settings[SettingKeyLLMMonitorStatusAPIURL]); value != "" {
		return value
	}
	if s.cfg != nil && strings.TrimSpace(s.cfg.LLMMonitor.StatusAPIURL) != "" {
		return strings.TrimSpace(s.cfg.LLMMonitor.StatusAPIURL)
	}
	return "https://jc.findcg.com/api/status"
}

func (s *SettingService) llmMonitorTitle(settings map[string]string) string {
	if value := strings.TrimSpace(settings[SettingKeyLLMMonitorTitle]); value != "" {
		return value
	}
	if s.cfg != nil && strings.TrimSpace(s.cfg.LLMMonitor.Title) != "" {
		return strings.TrimSpace(s.cfg.LLMMonitor.Title)
	}
	return "蛋云AI - Claude Code 监控面板"
}

func (s *SettingService) llmMonitorProviderURL(settings map[string]string) string {
	if value := strings.TrimSpace(settings[SettingKeyLLMMonitorProviderURL]); value != "" {
		return value
	}
	if s.cfg != nil && strings.TrimSpace(s.cfg.LLMMonitor.ProviderURL) != "" {
		return strings.TrimSpace(s.cfg.LLMMonitor.ProviderURL)
	}
	return "https://api.sunshinelink.online/"
}
