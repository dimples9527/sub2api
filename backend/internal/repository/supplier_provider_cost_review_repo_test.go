...
func TestSupplierProviderCostReviewRepositoryList(t *testing.T) {
	...
	mock.ExpectQuery(`SELECT r\.id, r\.provider_id, p\.name AS provider_name`).
		...
		WillReturnRows(costReviewRows().AddRow(
			...
			1.2, 1.0, 1.0, nil, 1.0, 0.2, 0.0, "pending_review", "none", nil, nil, int64(1), nil, nil, int64(1),
...
func TestSupplierProviderCostReviewRepositoryHistoryOrdersNewestFirst(t *testing.T) {
	...
	mock.ExpectQuery(`SELECT id, review_id, provider_id, stat_date`).
		...
		WillReturnRows(historyRows().AddRow(
			...
			1.2, 1.0, 1.0, 1.2, 0.2, 0.2, "approved", "upstream", nil, int64(9), time.Date(2026, 8, 1, 2, 0, 0, 0, time.UTC),
		))
...
func TestSupplierProviderCostReviewRepositoryApproveRollsBackWhenDailyStatsFails(t *testing.T) {
	...
	mock.ExpectQuery(`SELECT r\.id, r\.provider_id, p\.name AS provider_name.*FOR UPDATE`).
		...
		WillReturnRows(costReviewRows().AddRow(
			int64(10), int64(8), "supplier-a", time.Date(2026, 8, 1, 0, 0, 0, 0, time.UTC),
			1.2, 1.0, 1.0, nil, 1.0, 0.2, 0.0, "pending_review", "none", nil, nil, int64(1), nil, nil, int64(3),
			time.Date(2026, 8, 1, 0, 0, 0, 0, time.UTC), time.Date(2026, 8, 1, 0, 0, 0, 0, time.UTC),
		))
...
func TestSupplierProviderCostReviewRepositoryApproveRejectsVersionConflict(t *testing.T) {
	...
	mock.ExpectQuery(`SELECT r\.id, r\.provider_id, p\.name AS provider_name.*FOR UPDATE`).
		...
		WillReturnRows(costReviewRows().AddRow(
			int64(10), int64(8), "supplier-a", time.Date(2026, 8, 1, 0, 0, 0, 0, time.UTC),
			1.2, 1.0, 1.0, nil, 1.0, 0.2, 0.0, "pending_review", "none", nil, nil, int64(1), nil, nil, int64(4),
			time.Date(2026, 8, 1, 0, 0, 0, 0, time.UTC), time.Date(2026, 8, 1, 0, 0, 0, 0, time.UTC),
		))
	mock.ExpectRollback()
...
func TestSupplierProviderCostReviewRepositoryApproveManyUpdatesAllRecordsInOneTransaction(t *testing.T) {
	...
	mock.ExpectQuery(`SELECT r\.id, r\.provider_id, p\.name AS provider_name.*FOR UPDATE`).WithArgs(int64(10)).
		WillReturnRows(costReviewRows().AddRow(
			int64(10), int64(8), "supplier-a", now, 1.2, 1.0, 1.0, nil, 1.0, 0.2, 0.0, service.CostReviewStatusPending, service.CostReviewDecisionNone, nil, nil, int64(1), nil, now, int64(3), now, now,
		))
	...
	mock.ExpectExec(`INSERT INTO supplier_provider_daily_stats`).WithArgs(int64(8), "2026-08-01", 1.2).WillReturnResult(sqlmock.NewResult(1, 1))
	...
	mock.ExpectQuery(`SELECT r\.id, r\.provider_id, p\.name AS provider_name.*FOR UPDATE`).WithArgs(int64(11)).
		WillReturnRows(costReviewRows().AddRow(
			int64(11), int64(9), "supplier-b", now, 2.2, 2.0, 2.0, nil, 2.0, 0.2, 0.0, service.CostReviewStatusChanged, service.CostReviewDecisionCalculated, nil, nil, int64(2), nil, now, int64(7), now, now,
		))
	...
	mock.ExpectExec(`INSERT INTO supplier_provider_daily_stats`).WithArgs(int64(9), "2026-08-01", 2.2).WillReturnResult(sqlmock.NewResult(1, 1))
	mock.ExpectExec(`INSERT INTO supplier_provider_cost_review_histories`).WillReturnResult(sqlmock.NewResult(1, 1))
	mock.ExpectCommit()
...
func TestSupplierProviderCostReviewRepositoryApproveManyRollsBackWhenSecondVersionConflicts(t *testing.T) {
	...
	mock.ExpectQuery(`SELECT r\.id, r\.provider_id, p\.name AS provider_name.*FOR UPDATE`).WithArgs(int64(11)).
		WillReturnRows(costReviewRows().AddRow(
			int64(11), int64(9), "supplier-b", now, 2.2, 2.0, 2.0, nil, 2.0, 0.2, 0.0, service.CostReviewStatusPending, service.CostReviewDecisionNone, nil, nil, int64(2), nil, now, int64(8), now, now,
		))
	mock.ExpectRollback()
...
func TestSupplierProviderCostReviewRepositorySyncCreatesReviewAndUpdatesDailyStatsInTransaction(t *testing.T) {
	...
	mock.ExpectQuery(`INSERT INTO supplier_provider_cost_reviews`).WithArgs(
		int64(8), "2026-08-24", 1.2, 1.0, 1.0, 1.0, 0.2, 0.0, int64(7), now,
	...
	got, err := repo.Sync(context.Background(), service.SupplierProviderCostReviewSyncInput{ProviderID: 8, StatDate: now, UpstreamCost: ptr(1.2), CalculatedCost: ptr(1.0), AutoAdoptedCost: ptr(1.0), EffectiveCost: 1.0, SyncRunID: ptrInt64(7), SyncedAt: now})
	require.NoError(t, err)
...
func TestSupplierProviderCostReviewRepositorySyncPreservesApprovedCost(t *testing.T) {
	...
	mock.ExpectQuery(`SELECT r\.id, r\.provider_id, p\.name AS provider_name.*FOR UPDATE`).WithArgs(int64(8), "2026-08-24").
		WillReturnRows(costReviewRows().AddRow(
			...
			1.2, 1.0, 1.0, 1.2, 1.2, 0.2, 0.2, service.CostReviewStatusApproved, service.CostReviewDecisionUpstream, int64(9), now, int64(1), nil, now, int64(3), now, now,
		))
	mock.ExpectExec(`UPDATE supplier_provider_cost_reviews`).WithArgs(
		1.4, 1.1, 1.1, 0.3, 1.2, 0.2, service.CostReviewStatusChanged, int64(7), now, int64(10),
	).WillReturnResult(sqlmock.NewResult(1, 1))
	mock.ExpectExec(`INSERT INTO supplier_provider_cost_review_histories`).WillReturnResult(sqlmock.NewResult(1, 1))
	mock.ExpectCommit()

	got, err := repo.Sync(context.Background(), service.SupplierProviderCostReviewSyncInput{
		ProviderID: 8, StatDate: now, UpstreamCost: ptr(1.4), CalculatedCost: ptr(1.1), AutoAdoptedCost: ptr(1.1), EffectiveCost: 1.1, SyncRunID: ptrInt64(7), SyncedAt: now,
	})
	require.NoError(t, err)
...
func TestSupplierProviderCostReviewRepositorySyncRefreshesPendingEffectiveCost(t *testing.T) {
	...
	mock.ExpectQuery(`SELECT r\.id, r\.provider_id, p\.name AS provider_name.*FOR UPDATE`).WithArgs(int64(8), "2026-08-24").
		WillReturnRows(costReviewRows().AddRow(
			int64(10), int64(8), "supplier-a", now,
			1.2, 1.0, 1.0, nil, 1.0, 0.2, 0.0, service.CostReviewStatusPending, service.CostReviewDecisionNone, nil, nil, int64(1), nil, now, int64(3), now, now,
		))
	mock.ExpectExec(`UPDATE supplier_provider_cost_reviews`).WithArgs(
		1.4, 1.1, 1.1, 0.3, 1.1, 0.0, service.CostReviewStatusPending, int64(7), now, int64(10),
	).WillReturnResult(sqlmock.NewResult(1, 1))
...
func TestApplySyncToReviewKeepsApprovedStatusWhenLatestDataIsUnchanged(t *testing.T) {
	...
	got := applySyncToReview(review, service.SupplierProviderCostReviewSyncInput{
		UpstreamCost: upstreamPtr(upstream), CalculatedCost: upstreamPtr(calculated), AutoAdoptedCost: upstreamPtr(autoAdopted), EffectiveCost: 1.0, SyncedAt: now,
	})
...
func costReviewRows() *sqlmock.Rows {
	return sqlmock.NewRows([]string{"id", "provider_id", "provider_name", "stat_date", "upstream_cost", "calculated_cost", "auto_adopted_cost", "final_cost", "effective_cost", "cost_delta", "effective_delta", "status", "decision_type", "approved_by", "approved_at", "sync_count", "last_sync_run_id", "last_synced_at", "version", "created_at", "updated_at"})
}
func historyRows() *sqlmock.Rows {
	return sqlmock.NewRows([]string{"id", "review_id", "provider_id", "stat_date", "event_type", "sync_run_id", "upstream_cost", "calculated_cost", "auto_adopted_cost", "final_cost", "cost_delta", "effective_delta", "status", "decision_type", "manual_cost", "operator_id", "operated_at"})
}
