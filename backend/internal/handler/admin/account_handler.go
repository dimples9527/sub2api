...
func (h *AccountHandler) SetMonitorGroupPlatformOverrideService(overrideService service.MonitorGroupPlatformOverrideService) {
...
func (h *AccountHandler) ListModelSquareSyncAccounts(c *gin.Context) {
	...
	for _, group := range groups {
		...
		if actual := strings.ToLower(strings.TrimSpace(overrides[group.ID])); actual != "" {
			effective = actual
		}
