...
// 真实实现会走一次 HTTP 请求，测试里必须立即返回：KeysView 的 loadGroups 把它和
// getAvailable 放在同一个 Promise.all 里，慢一拍就导致弹窗打开时分组列表还是空的。
vi.mock('@/features/model-monitor/groupBusinessPlatformData', () => ({
  loadGroupBusinessPlatformMap: vi.fn().mockResolvedValue(new Map()),
}))
  ...
  const wrapper = mount(KeysView, {
    global: {
      stubs: {
        ...
        // KeyFormModal 不 stub：创建弹窗的厂商分类、分组联动与「重置额度后同步状态」
        // 都发生在组件内部，stub 掉就只剩空壳，这些断言会失去意义。
...
describe('user KeysView column settings', () => {
  ...
  describe('create provider selection', () => {
    ...
    it('classifies all configured platforms and retains the complete table filter', async () => {
      ...
      // fixture 的倍率全为 1，并列时落到名称排序，中文 locale 下「10」「11」排在「2」前面。
      expect(optionIds(wrapper)).toEqual([10, 11, 7, 8, 9])
      expect(wrapper.findAllComponents({ name: 'Select' })[0].props('options')).toHaveLength(13)
    })

    it('sorts the groups of the selected provider by rate ascending', async () => {
      // 倍率是主排序键：便宜的分组排前面，用户不用翻到最后才知道哪个划算。
      getAvailableGroups.mockResolvedValue([
        { id: 21, name: '贵分组', platform: 'kimi', rate_multiplier: 3, subscription_type: 'standard' },
        { id: 22, name: '便宜分组', platform: 'kimi', rate_multiplier: 0.5, subscription_type: 'standard' },
        { id: 23, name: '中等分组', platform: 'kimi', rate_multiplier: 1, subscription_type: 'standard' },
      ])
      const wrapper = await openCreate()
      expect(optionIds(wrapper)).toEqual([22, 23, 21])
    })
