...
export interface SupplierGroupMonitorTrendRow {
  provider: string
  availability: number
  latency: number
  time: string
  trend: SupplierGroupMonitorTrendPoint[]
}

export function normalizeSupplierGroupMonitorKey(value: unknown): string {
  return String(value ?? '').trim().toLowerCase().replace(/[\s_-]+/g, '')
}

export function buildSupplierGroupMonitorTrendIndex(payload: unknown): Map<string, SupplierGroupMonitorTrendRow> {
  const rows = normalizeMonitorPayload(payload)
  const index = new Map<string, SupplierGroupMonitorTrendRow>()
...
function normalizeMonitorItem(item: unknown, itemIndex: number): SupplierGroupMonitorTrendRow {
...
function rowKeys(provider: string, item: unknown): string[] {
  ...
  return keys.map(normalizeSupplierGroupMonitorKey).filter(Boolean)
}

function normalizeTrendPoint(point: unknown, row: { availability: number; latency: number; time: string }, fallbackIndex: number): SupplierGroupMonitorTrendPoint {
  if (isRecord(point)) {
...
function fallbackTrend(item: Record<string, unknown>, availability: number): unknown[] {
  ...
  const tone: SupplierGroupMonitorTone = availability >= 75 ? 'green' : availability >= 30 ? 'yellow' : 'red'
  return Array.from({ length: 18 }, () => tone)
}

function pointTone(point: Record<string, unknown>): SupplierGroupMonitorTone {
...
function normalizeTone(value: unknown): SupplierGroupMonitorTone {
...
function statusTextFromTone(tone: SupplierGroupMonitorTone): string {
