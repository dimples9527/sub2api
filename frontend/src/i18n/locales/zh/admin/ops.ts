export default {
    ops: {
      ...
      runtime: {
        ...
        slaMinPercentHint: 'SLA 低于该值时将以红色显示。默认：99.5%。',
        ttftP99MaxMs: 'TTFT P99 最大值（ms）',
        ttftP99MaxMsHint: 'TTFT P99 高于该值时将以红色显示。默认：500ms。',
        requestErrorRateMaxPercent: '请求错误率最大值（%）',
        requestErrorRateMaxPercentHint: '请求错误率高于该值时将以红色显示。默认：5%。',
        upstreamErrorRateMaxPercent: '上游错误率最大值（%）',
        upstreamErrorRateMaxPercentHint: '上游错误率高于该值时将以红色显示。默认：5%。',
        validation: {
          title: '请先修正以下问题',
          invalid: '设置不合法',
